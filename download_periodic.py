...
for tag in fl.ref_tags:
    ...
    mstar = E.readArray("SUBFIND", fl.ref_directory, tag, "/Subhalo/Stars/Mass", numThreads=1, noH=True)
    sfr = E.readArray("SUBFIND", fl.ref_directory, tag, "/Subhalo/StarFormationRate", numThreads=1, noH=True)

    write_data_h5py(fname, 'ref/mstar', tag, data=mstar, overwrite=True)
    write_data_h5py(fname, 'ref/sfr', tag, data=sfr, overwrite=True)

    mstar = E.readArray("SUBFIND", fl.agn_directory, tag, "/Subhalo/Stars/Mass", numThreads=1, noH=True)
    sfr = E.readArray("SUBFIND", fl.agn_directory, tag, "/Subhalo/StarFormationRate", numThreads=1, noH=True)
